...
type ControllerFetcher interface {
	...
	// GetWorkloadContainersRequest returns workloads container request resources
	GetWorkloadContainersRequest(controller *ControllerKeyWithAPIVersion) (map[string]corev1.ResourceList, bool)
}
...
func (f *controllerFetcher) periodicallyRefreshCache(ctx context.Context, period time.Duration) {
	for {
		...
		case <-time.After(period):
			...
			klog.V(3).Info("Starting to refresh entries in controllerFetchers scaleSubresourceCacheStorage")
			...
			klog.V(3).Infof("Finished refreshing %d entries in controllerFetchers scaleSubresourceCacheStorage", len(keysToRefresh))
...
func NewControllerFetcher(config *rest.Config, kubeClient kube_client.Interface, factory informers.SharedInformerFactory,
	kruiseFactory kruise_informer.SharedInformerFactory, betweenRefreshes, lifeTime time.Duration, jitterFactor float64, supportKruise bool) *controllerFetcher {
...
func getParentOfWellKnownController(informer cache.SharedIndexInformer, controllerKey ControllerKeyWithAPIVersion) (*ControllerKeyWithAPIVersion, error) {
	...
	case *appsv1.DaemonSet:
		return getOwnerController(obj.OwnerReferences, namespace), nil
...
func (f *controllerFetcher) FindTopMostWellKnownOrScalable(key *ControllerKeyWithAPIVersion) (*ControllerKeyWithAPIVersion, error) {
	...
	visited[*key] = true
	for {
		if common.IsTopmostKubeWorkload(key.ApiVersion, key.Kind) {
...
func (f *controllerFetcher) GetWorkloadContainersRequest(key *ControllerKeyWithAPIVersion) (map[string]corev1.ResourceList, bool) {
	informer, kindExists := f.informersMap[wellKnownController(key.Kind)]
	if !kindExists {
		return nil, false
	}
	obj, exists, err := informer.GetStore().GetByKey(key.Namespace + "/" + key.Name)
	if !exists || err != nil {
		return nil, false
	}
	switch obj.(type) {
	case *appsv1.DaemonSet:
		workload := obj.(*appsv1.DaemonSet)
		return getContainerRequestsFromPodTemplate(workload.Spec.Template), true
	case *appsv1.Deployment:
		workload := obj.(*appsv1.Deployment)
		return getContainerRequestsFromPodTemplate(workload.Spec.Template), true
	case *appsv1.StatefulSet:
		workload := obj.(*appsv1.StatefulSet)
		return getContainerRequestsFromPodTemplate(workload.Spec.Template), true
	default:
		return nil, false
	}
...
func getContainerRequestsFromPodTemplate(podTemplate corev1.PodTemplateSpec) map[string]corev1.ResourceList {
	containerRequests := map[string]corev1.ResourceList{}
	for _, c := range podTemplate.Spec.Containers {
		containerRequests[c.Name] = c.Resources.Requests
	}
