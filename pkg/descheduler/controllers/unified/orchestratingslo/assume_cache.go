...
type AssumeCache interface {
	...
	// ListByIndex lists all the objects by the index in the cache
	ListByIndex(indexName string, obj interface{}) []interface{}
}
...
func objInfoIndexAdaptor(indexFunc cache.IndexFunc) func(obj interface{}) ([]string, error) {
	return func(obj interface{}) ([]string, error) {
		objInfo, ok := obj.(*objInfo)
		if !ok {
			return []string{""}, &errWrongType{"objInfo", obj}
		}
		return indexFunc(objInfo.latestObj)
	}
}

// NewAssumeCache creates an assume cache for general objects.
func NewAssumeCache(informer controllercache.Informer, description string, indexers cache.Indexers) AssumeCache {
	c := &assumeCache{
		description: description,
	}
	indexersClone := make(cache.Indexers)
	for indexName, indexFunc := range indexers {
		indexersClone[indexName] = objInfoIndexAdaptor(indexFunc)
	}
	c.store = cache.NewIndexer(objInfoKeyFunc, indexersClone)
...
func (c *assumeCache) ListByIndex(indexName string, obj interface{}) []interface{} {
	c.rwMutex.RLock()
	defer c.rwMutex.RUnlock()

	allObjs := []interface{}{}
	objs, err := c.store.Index(indexName, &objInfo{latestObj: obj})
	if err != nil {
...
// PodAssumeCache is a AssumeCache for Pod objects
type PodAssumeCache interface {
	AssumeCache

	// GetPod returns the pod from the cache with given key.
	// key is the result of MetaNamespaceKeyFunc on Pod obj
	GetPod(key string) (*v1.Pod, error)
	GetAPIPod(key string) (*v1.Pod, error)
}

type podAssumeCache struct {
	AssumeCache
}

// NewPodAssumeCache creates a Pod assume cache.
func NewPodAssumeCache(informer controllercache.Informer) PodAssumeCache {
	return &podAssumeCache{NewAssumeCache(informer, "v1.Pod", nil)}
}

func (c *podAssumeCache) GetPod(key string) (*v1.Pod, error) {
	obj, err := c.Get(key)
	if err != nil {
		return nil, err
	}
	...
	pod, ok := obj.(*v1.Pod)
	if !ok {
		return nil, &errWrongType{"v1.Pod", obj}
	}
	return pod, nil
}

func (c *podAssumeCache) GetAPIPod(key string) (*v1.Pod, error) {
