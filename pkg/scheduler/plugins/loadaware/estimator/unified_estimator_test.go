...
func TestUnifiedEstimatorEstimatePod(t *testing.T) {
	tests := []struct {
		name          string
		pod           *corev1.Pod
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			estimator, err := NewUnifiedEstimator(&loadAwareSchedulingArgs, nil)
			assert.NoError(t, err)
			assert.NotNil(t, estimator)
			assert.Equal(t, unifiedEstimatorName, estimator.Name())
...
func TestUnifiedEstimatorEstimateNode(t *testing.T) {
	tests := []struct {
		name string
		node *corev1.Node
			...
			node: &corev1.Node{
				ObjectMeta: metav1.ObjectMeta{
					Annotations: map[string]string{
						unified.AnnotationOriginalNodeAllocatable: `{"cpu":28,"memory":"32Gi"}`,
					},
				},
				Status: corev1.NodeStatus{
					Allocatable: corev1.ResourceList{
