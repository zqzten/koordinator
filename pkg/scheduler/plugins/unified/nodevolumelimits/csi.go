...
const CSIName = "Unified" + names.NodeVolumeLimits
...
func (pl *CSILimits) Filter(ctx context.Context, cycleState *framework.CycleState, pod *v1.Pod, nodeInfo *framework.NodeInfo) *framework.Status {
	var requestsVolumeCounts map[string]int
	if reservationutil.IsReservePod(pod) {
		var status *framework.Status
		if requestsVolumeCounts, status = pl.getReservationVolumeCounts(pod, nodeInfo); !status.IsSuccess() {
			return status
		}
	}

	// If the new pod doesn't have any volume attached to it, the predicate will always be true
	if len(pod.Spec.Volumes) == 0 && len(requestsVolumeCounts) == 0 {
		return nil
	}
	...
	if len(newVolumes) == 0 && len(requestsVolumeCounts) == 0 {
		return nil
	}
	...
	rrState := getReservationRestoreState(cycleState).getNodeState(node.Name)
	unmatchedReservedVolumeLimits, err := pl.getReservationVolumeLimitKey(pod.Namespace, csiNode, rrState.unmatchedReservedVolumeLimits)
	if err != nil {
		return framework.NewStatus(framework.UnschedulableAndUnresolvable, err.Error())
	}

	attachedVolumes := make(map[string]string)
	attachedVolumesInRR := make(map[string]string)
	for _, existingPod := range nodeInfo.Pods {
		if rrState.unmatchedReservedVolumeLimits == nil && reservationutil.IsReservePod(existingPod.Pod) {
			counts, status := pl.getReservationVolumeCounts(existingPod.Pod, nodeInfo)
			if !status.IsSuccess() {
				return status
			}
			if unmatchedReservedVolumeLimits == nil {
				unmatchedReservedVolumeLimits = counts
			} else {
				for k, v := range counts {
					unmatchedReservedVolumeLimits[k] = v
				}
			}
		}
		result := map[string]string{}
		if err := pl.filterAttachableVolumes(existingPod.Pod, csiNode, false /* existing pod */, result); err != nil {
			return framework.AsStatus(err)
		}

		rInfo, status := pl.getReservationInfoByPod(rrState, existingPod.Pod, node.Name)
		if !status.IsSuccess() {
			return status
		}
		for k, v := range result {
			attachedVolumes[k] = v
			if rInfo != nil {
				attachedVolumesInRR[k] = v
			}
		}
	}

	attachedVolumeCount := map[string]int{}
	attachedReservedVolumeCounts := map[string]int{}
	for volumeUniqueName, volumeLimitKey := range attachedVolumes {
		...
		if _, ok := attachedVolumesInRR[volumeUniqueName]; ok {
			attachedReservedVolumeCounts[volumeLimitKey]++
		}
	}
	...
	for volumeLimitKey, volumeReservationCount := range requestsVolumeCounts {
		count := newVolumeCount[volumeLimitKey]
		if count < volumeReservationCount {
			newVolumeCount[volumeLimitKey] = volumeReservationCount
		}
	}
	...
	for volumeLimitKey, count := range newVolumeCount {
		...
		if ok {
			reservedVolumeLimit := unmatchedReservedVolumeLimits[volumeLimitKey]
			attachedReservedVolumeCount := attachedReservedVolumeCounts[volumeLimitKey]
			if reservedVolumeLimit > attachedReservedVolumeCount {
				reservedVolumeLimit = reservedVolumeLimit - attachedReservedVolumeCount
			} else {
				reservedVolumeLimit = 0
			}
			currentVolumeCount := attachedVolumeCount[volumeLimitKey] + reservedVolumeLimit
