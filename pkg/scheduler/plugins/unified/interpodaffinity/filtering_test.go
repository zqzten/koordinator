...
func TestRequiredAffinitySingleECINode(t *testing.T) {
	podLabel := map[string]string{"service": "securityscan"}
	labels1 := map[string]string{
		"region":             "r1",
		"zone":               "z11",
	...
	tests := []struct {
		...
		{
			name: "[PASS Because ECI] Does not satisfy the PodAffinity with labelSelector because of diff Namespace",
			pod: createPodWithAffinityTerms(defaultNamespace, "", podLabel2,
				[]v1.PodAffinityTerm{
					{
						LabelSelector: &metav1.LabelSelector{
		...
		{
			name: "[PASS Because ECI] Doesn't satisfy the PodAffinity because of unmatching labelSelector with the existing pod",
			pod: createPodWithAffinityTerms(defaultNamespace, "", podLabel,
				[]v1.PodAffinityTerm{
					{
						LabelSelector: &metav1.LabelSelector{
		...
		{
			name: "[PASS Because ECI] satisfies the PodAffinity but doesn't satisfy the PodAntiAffinity with the existing pod",
			pod: createPodWithAffinityTerms(defaultNamespace, "", podLabel2,
				[]v1.PodAffinityTerm{
					{
						LabelSelector: &metav1.LabelSelector{
...
func TestRequiredAffinityMultipleNodes(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			name: "The first pod of the collection can only be scheduled on nodes labeled with the requested topology keys",
		},
		{
...
func TestPreFilterDisabled(t *testing.T) {
	...
	p := plugintesting.SetupPlugin(t, n, &config.InterPodAffinityArgs{}, NewSnapshot(nil, nil))
	cycleState := framework.NewCycleState()
	gotStatus := p.(framework.FilterPlugin).Filter(context.Background(), cycleState, pod, nodeInfo)
	wantStatus := framework.AsStatus(fmt.Errorf(`error reading "PreFilterUnifiedInterPodAffinity" from cycleState: %w`, framework.ErrNotFound))
...
func TestPreFilterStateAddRemovePod(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			getState := func(pods []*v1.Pod) (*InterPodAffinity, *framework.CycleState, *preFilterState, framework.SharedLister) {
				snapshot := NewSnapshot(pods, test.nodes)
				n := func(plArgs runtime.Object, fh framework.Handle) (framework.Plugin, error) {
					return NewWithFeature(plArgs, fh, feature.Features{})
				}
...
func mustGetNodeInfo(t *testing.T, snapshot framework.SharedLister, name string) *framework.NodeInfo {
	t.Helper()
