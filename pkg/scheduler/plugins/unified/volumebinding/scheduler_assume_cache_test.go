...
func makePVWithAnnotationAndPhase(name, version, storageClass string, annotation map[string]string, phase v1.PersistentVolumePhase) *v1.PersistentVolume {
...
func makeLocalPV(name, version, selectedNode, storageClass, capacity string, claim *v1.PersistentVolumeClaim) *v1.PersistentVolume {
	pv := &v1.PersistentVolume{
		ObjectMeta: metav1.ObjectMeta{
			Name:            name,
			ResourceVersion: version,
...
func verifyListPVs(t *testing.T, cache PVAssumeCache, expectedPVs map[string]*v1.PersistentVolume, storageClassName string) {
	pvList := cache.ListPVs("", storageClassName, v1.VolumeAvailable)
