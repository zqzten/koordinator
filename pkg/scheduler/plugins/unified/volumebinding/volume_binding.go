...
const Name = "UnifiedVolumeBinding"
...
func (pl *VolumeBinding) Reserve(ctx context.Context, cs *framework.CycleState, pod *v1.Pod, nodeName string) *framework.Status {
	...
	pl.assumeEphemeralStorage(pod, nodeName)
	...
	if ok {
		...
		err = pl.assumeLocalPVCAllocs(podVolumes, nodeName, pod)
		if err != nil {
...
func (pl *VolumeBinding) Unreserve(ctx context.Context, cs *framework.CycleState, pod *v1.Pod, nodeName string) {
	...
	pl.revertAssumedEphemeralStorage(pod, nodeName)
...
func New(plArgs runtime.Object, fh framework.Handle) (framework.Plugin, error) {
	defaultVolumeBindingArgs, err := getDefaultVolumeBindingArgs()
	if err != nil {
		return nil, err
	}

	if plArgs != nil {
		switch t := plArgs.(type) {
		case *config.VolumeBindingArgs:
			defaultVolumeBindingArgs = t
		case *runtime.Unknown:
			unknownObj := t
			if err := frameworkruntime.DecodeInto(unknownObj, defaultVolumeBindingArgs); err != nil {
				return nil, err
			}
		default:
			return nil, fmt.Errorf("got args of type %T, want *VolumeBindingArgs", plArgs)
		}
	}
	plArgs = defaultVolumeBindingArgs
	...
	registerPVCEventHandler(fh.SharedInformerFactory(), nodeStorageCache)
	registerPodEventHandler(fh.SharedInformerFactory(), nodeStorageCache)
	...
	binder := NewVolumeBinder(fh.ClientSet(), podInformer, nodeInformer, csiNodeInformer, pvcInformer, pvInformer, storageClassInformer, capacityCheck, time.Duration(args.BindTimeoutSeconds)*time.Second, fh.EventRecorder(), nodeStorageCache)
