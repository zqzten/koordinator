...
func NewAssumeCache(informer cache.SharedIndexInformer, description string, indexers cache.Indexers) AssumeCache {
	c := &assumeCache{
...
type PVAssumeCache interface {
	...
	ListPVs(nodeName string, storageClassName string, phase v1.PersistentVolumePhase) []*v1.PersistentVolume
}
...
func NewPVAssumeCache(informer cache.SharedIndexInformer) PVAssumeCache {
	return &pvAssumeCache{NewAssumeCache(informer, "v1.PersistentVolume", persistentVolumeIndexers)}
}
...
func (c *pvAssumeCache) ListPVs(nodeName string, storageClassName string, phase v1.PersistentVolumePhase) []*v1.PersistentVolume {
	indexName := persistentVolumeIndex
	if nodeName != "" {
		indexName = persistentVolumeIndexWithNodeName
	}
	...
	for _, obj := range objs {
		pv, ok := obj.(*v1.PersistentVolume)
		if !ok {
			klog.Errorf("ListPVsWithNodeName: %v", &errWrongType{"v1.PersistentVolume", obj})
			continue
		}
		pvs = append(pvs, pv)
	}
