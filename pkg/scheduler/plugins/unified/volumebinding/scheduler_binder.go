...
const (
	...
	// ErrReasonNotEnoughIOLimit is used when a node volume ioLimit is not satisfied
	ErrReasonNotEnoughIOLimit = "node(s) did not have enough iolimit"
)
...
type BindingInfo struct {
	...
	// Matched local disk mount point
	bindPoint *BindPoint
}
...
type PodVolumes struct {
	...
	// AssumedLocalPVCAllocs are assumed localPVCAllocs
	AssumedLocalPVCAllocs []*localPVCAlloc
}
...
func NewVolumeBinder(
	...
	b.nodeStorageCache = nodeStorageCache
...
func (b *volumeBinder) FindPodVolumes(pod *v1.Pod, boundClaims, claimsToBind []*v1.PersistentVolumeClaim, node *v1.Node) (podVolumes *PodVolumes, reasons ConflictReasons, err error) {
	...
	defer func() {
		...
		if !sufficientIOLimit {
			reasons = append(reasons, ErrReasonNotEnoughIOLimit)
		}
	}()
	...
	if len(claimsToBind) > 0 {
		...
		if len(claimsToProvision) > 0 || len(staticBindings) > 0 {
			unboundVolumesSatisfied, sufficientStorage, sufficientIOLimit, dynamicProvisions, err = b.checkVolumeProvisions(pod, staticBindings, claimsToProvision, node)
			if err != nil {
				return
			}
		}
	...
	for _, binding := range podVolumes.StaticBindings {
		if binding.pv != nil {
			...
			klog.V(5).Infof("AssumePodVolumes: GetBindVolumeToClaim for pod %q, PV %q, PVC %q.  newPV %p, dirty %v, err: %v",
				...
				err)
			if err != nil {
				klog.Errorf("AssumePodVolumes: failed to GetBindVolumeToClaim, pod: %v, pvc: %s/%s, pv: %v, err: %v",
					podName, binding.pvc.Namespace, binding.pvc.Name, binding.pv.Name, err)
				b.revertAssumedPVs(newBindings)
				b.revertAssumedPVCs(newNodeDiskProvisionedPVCs)
				return false, err
			}
			...
			if dirty {
				...
				if err != nil {
					klog.Errorf("AssumePodVolumes: failed to assume, pod: %v, pvc: %s/%s, pv: %v, err: %v",
						podName, binding.pvc.Namespace, binding.pvc.Name, binding.pv.Name, err)
					b.revertAssumedPVs(newBindings)
					b.revertAssumedPVCs(newNodeDiskProvisionedPVCs)
					return false, err
			...
			newBindings = append(newBindings, &BindingInfo{pv: newPV, pvc: binding.pvc, bindPoint: binding.bindPoint})
		}
		if binding.bindPoint != nil {
			...
			if err != nil {
				klog.Errorf("AssumePodVolumes: failed to assume with multiLocalVolume, pod: %v, pvc: %s/%s, err: %v",
					podName, claimClone.Namespace, claimClone.Name, err)
				b.revertAssumedPVs(newBindings)
				b.revertAssumedPVCs(newNodeDiskProvisionedPVCs)
				return
			}
			klog.Infof("[MultiLocalVolumes], Update pod: %v binding info, node: %v, mount point: %v", podName, nodeName, binding.bindPoint.MountPoint)
			newBindings = append(newBindings, &BindingInfo{pv: binding.pv, pvc: claimClone, bindPoint: binding.bindPoint})
			newNodeDiskProvisionedPVCs = append(newNodeDiskProvisionedPVCs, claimClone)
		}
	}
...
func (b *volumeBinder) BindPodVolumes(assumedPod *v1.Pod, podVolumes *PodVolumes) (err error) {
	...
	err = b.bindAPIUpdate(assumedPod, podName, bindings, claimsToProvision)
	if err != nil {
		return err
	}
...
func (b *volumeBinder) bindAPIUpdate(assumedPod *v1.Pod, podName string, bindings []*BindingInfo, claimsToProvision []*v1.PersistentVolumeClaim) error {
	if bindings == nil {
	...
	var (
		bindingPVNames         []string
		i                      int
		claim                  *v1.PersistentVolumeClaim
		localClaimsToProvision []*v1.PersistentVolumeClaim
	)
	...
	for i, binding := range bindings {
		if binding.pv != nil {
			...
			bindingPVNames = append(bindingPVNames, binding.pv.Name)
		} else {
			claimKey := getPVCName(binding.pvc)
			klog.V(2).Infof("claim %q bound to delay provision volume", claimKey)
			if binding.bindPoint != nil {
				klog.V(4).Infof("bindAPIUpdate: Request %q, PVC %#v", podName, binding.pvc)
				newClaim, err := b.kubeClient.CoreV1().PersistentVolumeClaims(binding.pvc.Namespace).Update(context.TODO(), binding.pvc, metav1.UpdateOptions{})
				if err != nil {
					klog.Errorf("Failed to update PVC %s, Request %q, err: %v", binding.pvc.Name, podName, err)
					return err
				}
				// Save updated object from apiserver for later checking.
				bindings[i].pvc = newClaim
				localClaimsToProvision = append(localClaimsToProvision, newClaim)
				lastProcessedBinding++
			}
		}
	}
	if len(bindingPVNames) > 0 {
		b.recordEvent(assumedPod, v1.EventTypeNormal, "VolumeBinding", "Binding", "Start binding volume(s) %s", bindingPVNames)
	}
	...
	b.recordExternalProvisioningEvent(assumedPod, localClaimsToProvision)
	return nil
}
...
func (b *volumeBinder) checkBindings(pod *v1.Pod, bindings []*BindingInfo, claimsToProvision []*v1.PersistentVolumeClaim) (bool, error) {
	...
	// When pod is deleted, binding operation should be canceled. There is no
	...
	for _, binding := range bindings {
		if binding.bindPoint != nil {
			...
			if err != nil {
				return false, fmt.Errorf("fail to dynamically provisioned local pvc: %v, error: %v", binding.pvc.Name, err)
			}
			// Check if pvc is fully bound
			if !b.isPVCFullyBound(pvc) {
				klog.Infof("PVC %v is not fully bound", binding.pvc.Name)
				return false, nil
			}
...
func (b *volumeBinder) isVolumeBound(pod *v1.Pod, vol *v1.Volume) (bound bool, pvc *v1.PersistentVolumeClaim, err error) {
	...
	case vol.Ephemeral != nil:
		if !utilfeature.DefaultFeatureGate.Enabled(koordfeature.GenericEphemeralVolume) {
...
func (b *volumeBinder) checkBoundClaims(claims []*v1.PersistentVolumeClaim, node *v1.Node, podName string) (bool, bool, error) {
	...
	for _, pvc := range claims {
		...
		err = checkDiskAffinity(pv, node.Labels)
		if err != nil {
			klog.V(4).Infof("PersistentVolume %q, Node %q mismatch for Pod %q: %v", pvName, node.Name, podName, err)
			return false, true, nil
		}
...
func (b *volumeBinder) findMatchingVolumes(pod *v1.Pod, claimsToBind []*v1.PersistentVolumeClaim, node *v1.Node) (foundMatches bool, bindings []*BindingInfo, unboundClaims []*v1.PersistentVolumeClaim, err error) {
	...
	for _, pvc := range claimsToBind {
		if !unified.IsVirtualKubeletNode(node) && utilfeature.DefaultFeatureGate.Enabled(koordfeature.LocalDeviceVolume) {
			...
			if shouldPassFilter {
				if !isMatch {
					klog.V(2).Infof("[MultiLocalVolumes] No matching volumes for Request %q, PVC %q on node %q", podName, pvc.Name, node.Name)
					return false, nil, nil, fmt.Errorf("unbound claims failed to match %+v", pvc.Name)
				} else {
					bindPoints := bps[suggestedBP.MountPoint]
					bindPoints = append(bindPoints, suggestedBP)
					bps[suggestedBP.MountPoint] = bindPoints
					bindInfo := &BindingInfo{
						pvc:       pvc,
						bindPoint: suggestedBP,
					}
					bindings = append(bindings, bindInfo)
					klog.V(2).Infof("PVC suggest bind point %+v, pvc: %s/%s", suggestedBP, pvc.Namespace, pvc.Name)
				}
				continue
			}
		}
		...
		if unified.SupportLocalPV(b.classLister, storageClassName, nil) || unified.SupportLVMOrQuotaPathOrDevice(b.classLister, storageClassName, nil) {
			allPVs = b.pvCache.ListPVs(node.Name, storageClassName, v1.VolumeAvailable)
		} else {
			allPVs = b.pvCache.ListPVs("", storageClassName, v1.VolumeAvailable)
		}
		...
		if err = checkDiskAffinity(pv, node.Labels); err != nil {
			return false, nil, nil, err
		}
...
func (b *volumeBinder) checkVolumeProvisions(pod *v1.Pod, staticBindings []*BindingInfo, claimsToProvision []*v1.PersistentVolumeClaim, node *v1.Node) (provisionSatisfied, sufficientStorage, sufficientIOLimit bool, dynamicProvisions []*v1.PersistentVolumeClaim, err error) {
	podName := getPodName(pod)
	dynamicProvisions = []*v1.PersistentVolumeClaim{}
	var requestSystemDiskInBytes int64
	localVolumesInBytes := make(map[string]int64)
	requestLocalVolumeIOLimit := make(map[string]*unified.LocalStorageIOLimitInfo)

	klog.V(5).Infof("checkVolumeProvisions for pod: %s, on node : %s", podName, node.Name)

	// 通过 nodediskstorageclass 方式调度出来的 pvc bind 信息都记录在 staticBindings 中
	// 需要对这些 pvc 进行相关的资源检查
	for _, binding := range staticBindings {
		if binding.pvc == nil {
			continue
		}
		claim := binding.pvc
		// 对于 staticBindings 中没有 className 或者没有对应 storageclass 的 pvc 直接跳过，不做 iolimit 的检查
		...
		requestLocalVolumeIOLimit, err = b.addRequestedIoLimitFromPVC(claim, class, requestLocalVolumeIOLimit)
		if err != nil {
	...
	for _, claim := range claimsToProvision {
		...
		if err != nil {
			return false, false, true, nil, err
		}
		if !sufficient {
		...
		if ok, err := b.matchTopologyStorageClassNode(class, claim, node.Labels); err != nil {
			klog.V(4).Infof("Node %q claim: %q/%q matchTopologyStorageClassNode error: %v", node.Name, claim.Namespace, claim.Name, err)
		} else if !ok {
			klog.V(4).Infof("Node %q cannot satisfy provisioning topology requirements of claim %q", node.Name, pvcName)
			return false, true, true, nil, nil
		}

		// if the storageClass is LocalPV, then check the capacity
		if unified.SupportLocalPV(b.classLister, className, class) {
			quantity := claim.Spec.Resources.Requests[v1.ResourceStorage]
			requestSystemDiskInBytes += quantity.Value()
		}

		// if the storageClass is LVM or QuotaPath then check the capacity
	...
	// EnableLocalVolumeCapacity 默认打开，检查 LVM/QuotaPath 协议下的 pvc capacity 资源
	// 蚂蚁场景暂不使用 capacity 资源检查，但是需要复用 LVM/QuotaPath 协议进行 IOLimit 检查
	if !utilfeature.DefaultFeatureGate.Enabled(koordfeature.EnableLocalVolumeCapacity) {
		localVolumesInBytes = make(map[string]int64)
	}
	// 开了 IOLimitFeatureGate 才检查 ioLimit
	if !utilfeature.DefaultFeatureGate.Enabled(koordfeature.EnableLocalVolumeIOLimit) {
		requestLocalVolumeIOLimit = make(map[string]*unified.LocalStorageIOLimitInfo)
	}

	if requestSystemDiskInBytes > 0 || len(localVolumesInBytes) > 0 || len(requestLocalVolumeIOLimit) > 0 {
		sufficientStorage, sufficientIOLimit, err = b.checkLocalVolumeStorageAndIOCapacity(node.Name, pod, requestSystemDiskInBytes, localVolumesInBytes, requestLocalVolumeIOLimit)
		if !sufficientStorage || !sufficientIOLimit || err != nil {
			return false, sufficientStorage, sufficientIOLimit, nil, err
		}
	}

	if klog.V(5).Enabled() {
		klog.Infof("Provisioning for %d claims of Request %q that has no matching volumes on node %q ...", len(claimsToProvision), podName, node.Name)
	}

	return true, true, true, dynamicProvisions, nil
}

func (b *volumeBinder) revertAssumedPVs(bindingInfos []*BindingInfo) {
	for _, info := range bindingInfos {
		if info.bindPoint != nil {
			b.pvcCache.Restore(info.pvc.Name)
			continue
		}
		if info.pv != nil {
			b.pvCache.Restore(info.pv.Name)
		}
	}
}
