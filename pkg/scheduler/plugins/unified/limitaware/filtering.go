...
func (p *Plugin) Filter(ctx context.Context, cycleState *framework.CycleState, pod *corev1.Pod, nodeInfo *framework.NodeInfo) *framework.Status {
	...
	nodeLimitAllocated := p.nodeLimitsCache.GetAllocated(node.Name)
	insufficientResources := fitsLimit(limitToAllocatable, state.resource, nodeLimitAllocated, nodeLimitAllocatable)
...
func fitsLimit(resourceNameMap extension.LimitToAllocatable, podLimits, nodeLimitAllocated, nodeLimitAllocatable *framework.Resource) []noderesources.InsufficientResource {
	...
	if _, ok := resourceNameMap[corev1.ResourceCPU]; ok {
		if podLimits.MilliCPU > (nodeLimitAllocatable.MilliCPU - nodeLimitAllocated.MilliCPU) {
			insufficientResources = append(insufficientResources, noderesources.InsufficientResource{
				...
				Capacity:     nodeLimitAllocatable.MilliCPU,
			})
		}
	}
	if _, ok := resourceNameMap[corev1.ResourceMemory]; ok {
		if podLimits.Memory > (nodeLimitAllocatable.Memory - nodeLimitAllocated.Memory) {
			insufficientResources = append(insufficientResources, noderesources.InsufficientResource{
				ResourceName: corev1.ResourceMemory,
				Reason:       "Insufficient memory limit",
				Requested:    podLimits.Memory,
				Used:         nodeLimitAllocated.Memory,
				Capacity:     nodeLimitAllocatable.Memory,
			})
		}
	}
	if _, ok := resourceNameMap[corev1.ResourceEphemeralStorage]; ok {
		if podLimits.EphemeralStorage > (nodeLimitAllocatable.EphemeralStorage - nodeLimitAllocated.EphemeralStorage) {
			insufficientResources = append(insufficientResources, noderesources.InsufficientResource{
				...
				Capacity:     nodeLimitAllocatable.EphemeralStorage,
			})
		}
	}
	...
	for rName, rQuant := range podLimits.ScalarResources {
		...
		if _, ok := resourceNameMap[rName]; ok {
			if rQuant > (nodeLimitAllocatable.ScalarResources[rName] - limitAllocated) {
				insufficientResources = append(insufficientResources, noderesources.InsufficientResource{
					ResourceName: rName,
					...
					Capacity:     nodeLimitAllocatable.ScalarResources[rName],
				})
			}
		}
	}
