...
func (p *Plugin) Score(ctx context.Context, cycleState *framework.CycleState, pod *corev1.Pod, nodeName string) (int64, *framework.Status) {
	...
	limitToAllocatable, err := extension.GetNodeLimitToAllocatable(node.Annotations, p.defaultLimitToAllocatable)
	if err != nil {
		return 0, framework.AsStatus(err)
	}
	nodeLimitAllocatable, err := extension.GetNodeLimitAllocatable(nodeInfo.Allocatable, limitToAllocatable)
	if err != nil {
		return 0, framework.AsStatus(err)
	}
	nodeLimitAllocated := p.nodeLimitsCache.GetNonZeroAllocated(node.Name)
	nodeAllocated := make(resourceToValueMap)
	nodeAllocatable := make(resourceToValueMap)
	for resource := range p.resourceToWeightMap {
		if _, ok := limitToAllocatable[resource]; ok {
			allocatable, allocated := p.calculateResourceLimitAllocatableAllocated(nodeLimitAllocated, nodeLimitAllocatable, state.nonZeroResource, resource)
			if allocatable != 0 {
				// Only fill the extended resource entry when it's non-zero.
				nodeAllocatable[resource], nodeAllocated[resource] = allocatable, allocated
			}
		}
	}

	score := p.leastResourceScorer(nodeAllocated, nodeAllocatable)
	return score, nil
}

func (p *Plugin) calculateResourceLimitAllocatableAllocated(nodeLimitAllocated, nodeLimitAllocatable, podLimitRequested *framework.Resource, resourceName corev1.ResourceName) (int64, int64) {
	switch resourceName {
	case corev1.ResourceCPU:
		return nodeLimitAllocatable.MilliCPU, nodeLimitAllocated.MilliCPU + podLimitRequested.MilliCPU
	case corev1.ResourceMemory:
		return nodeLimitAllocatable.Memory, nodeLimitAllocated.Memory + podLimitRequested.Memory
	case corev1.ResourceEphemeralStorage:
		return nodeLimitAllocatable.EphemeralStorage, nodeLimitAllocated.EphemeralStorage + podLimitRequested.EphemeralStorage
	default:
		if _, exists := nodeLimitAllocatable.ScalarResources[resourceName]; exists {
			...
			return nodeLimitAllocatable.ScalarResources[resourceName], limitAllocated + podLimitRequested.ScalarResources[resourceName]
		}
	}
	return 0, 0
}
