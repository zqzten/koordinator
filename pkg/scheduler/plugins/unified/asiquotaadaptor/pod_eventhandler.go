...
func registerPodEventHandler(cache *ASIQuotaCache, factory informers.SharedInformerFactory) {
...
func (h *podEventHandler) OnUpdate(oldObj, newObj interface{}) {
	...
	if util.IsPodTerminated(newPod) {
		h.cache.deletePod(newPod)
		return
	}

	h.cache.updatePod(oldPod, newPod)
}
