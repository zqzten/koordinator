...
import (
	...
	"github.com/koordinator-sh/koordinator/pkg/scheduler/plugins/cnstack/firstfit/parallelize"
)

const (
	LabelFirstFitScheduling = "alibabacloud.com/first-fit-scheduling"
)

var _ scheduler.ScheduleAlgorithm = &firstFitScheduler{}

type firstFitScheduler struct {
	baseScheduleAlgorithm scheduler.ScheduleAlgorithm
	nextStartNodeIndex    int
}

// NewFirstFitScheduler creates a scheduler.ScheduleAlgorithm object.
func NewFirstFitScheduler(baseScheduleAlgorithm scheduler.ScheduleAlgorithm) scheduler.ScheduleAlgorithm {
	return &firstFitScheduler{
		baseScheduleAlgorithm: baseScheduleAlgorithm,
	}
}
...
func (g *firstFitScheduler) Schedule(ctx context.Context, extenders []framework.Extender, fwk framework.Framework, state *framework.CycleState, pod *corev1.Pod) (result scheduler.ScheduleResult, err error) {
	...
	if pod.Labels[LabelFirstFitScheduling] != "true" {
		return g.baseScheduleAlgorithm.Schedule(ctx, extenders, fwk, state, pod)
	}

	state.Write(firstFitDisableStateKey, nil)
	_, err = g.baseScheduleAlgorithm.Schedule(ctx, extenders, fwk, state, pod)
	state.Delete(firstFitDisableStateKey)

	if err != ErrFirstFitDisabled {
...
func (g *firstFitScheduler) selectHost(nodeScoreList framework.NodeScoreList) (string, error) {
...
func (g *firstFitScheduler) evaluateNominatedNode(ctx context.Context, extenders []framework.Extender, pod *corev1.Pod, fwk framework.Framework, state *framework.CycleState, diagnosis framework.Diagnosis) ([]*corev1.Node, error) {
...
func (g *firstFitScheduler) findNodesThatFitPod(ctx context.Context, extenders []framework.Extender, fwk framework.Framework, state *framework.CycleState, pod *corev1.Pod) ([]*corev1.Node, framework.Diagnosis, error) {
...
func (g *firstFitScheduler) findNodesThatPassFilters(
	...
	// NOTE(joseph.lt): FirstFit just need one node
